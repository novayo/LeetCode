class Solution:
    def minPushBox(self, grid: List[List[str]]) -> int:
        ...
        def get_score(step, x, y):
            return step + (abs(pos_t[0] - x) + abs(pos_t[1] - y))

        pq = [(get_score(0, pos_b[0], pos_b[1]), 0, pos_b[0], pos_b[1], pos_s[0], pos_s[1])]
        done_set = set()
        done_set.add((pos_b[0], pos_b[1], pos_s[0], pos_s[1]))

        while pq:
            score, step, i, j, s_i, s_j = heapq.heappop(pq)
            ...
            for x, y, s_x, s_y in [i-1,j,i+1,j],[i+1,j,i-1,j],[i,j-1,i,j+1],[i,j+1,i,j-1]:
                ...
                heapq.heappush(pq, (get_score(step+1, x, y), step+1, x, y, i, j))
